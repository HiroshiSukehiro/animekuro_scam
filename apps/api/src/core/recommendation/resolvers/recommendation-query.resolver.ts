...
import { GetAnimeByRecommendationResultsType } from '../models/results';
...
export class RecommendationQueryResolver extends RecommendationRootResolver {
    ...
    // @ResolveField(() => GetAnimeByRecommendationResultsType, {
    //     middleware: [AuthMiddleware],
    // })
    // async getAnimeByRecommendation(
    //     @Args('recommendation_count', { nullable: true }) take: number,
    // ): Promise<GetAnimeByRecommendationResultsType> {
    //     return await this.recommendationService.getAnimeByRecommendation(take);
    // }
}
