...
export class UserProfileService {
    ...
    async getGenresLikeFolders(
        ...
        if (toSort === 'count') result.sort(this.byField('count'));

        if (toSort === 'countIn') result.sort(this.byField('animes'));

        if (toSort === 'percent') result.sort(this.byField('percent'));

        if (sortBy === 'desc') result.reverse();
    ...
    async getStudiosLikeFolders(
        ...
        //@ts-ignore
        const studiosIds = Object.keys(statistic?.statistics.folder.studio);

        const countSum = Object.values(
            //@ts-ignore
            statistic?.statistics.folder.studio,
        ).reduce((a: any, b: any) => a + b, 0);
        ...
        studios.map((studio) =>
            result.push({
                ...
                //@ts-ignore
                count: statistic?.statistics.folder.studio[studio.id],
                percent:
                    //@ts-ignore
                    Math.round((statistic?.statistics.folder.studio[studio.id] / countSum).toFixed(2) * 100,
                    ),
                animes: studio.animes as any,
            }),
        );
